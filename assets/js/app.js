...
(function () {
  ...
  Promise.all([
    ...
    fetch("/data/summary_ja.json?v=1789002879", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
    fetch("/data/kuro_voice.json?v=1789002879", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
    fetch("/data/affiliates.json?v=1789002879", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
  ])
