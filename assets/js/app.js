...
(function () {
  ...
  let AFF = null; // 余韻ゾーンの送客枠（data/affiliates.json・未設定なら何も出ない）
  ...
  /* 冒頭の宣言文（「つまりこの3枚、こういう話です。」）と、締めの「——3枚が並べているのは、そういう流れです。」を
     出すかどうか。スプレッド別に切れる。
     経緯＝2026-09-19、BUBU が本番を触って「なんかいやだな」「同じ病気ｗ」。どちらも
     「これは3枚の話ですよ」というメタな説明で、画面にはもうカードが並んでいる＝絵が言っている事を言葉で言い直している。
     ✅ **2026-09-19 BUBU 承認済み（BUBU「あー、おｋ　それでいいよ」）＝3枚・6枚・2枚とも false（出さない）。**
     文言そのもの（meta.opener / meta.echo_tail / meta.choice）はデータ側に残してある＝true に戻せばいつでも復活する。 */
  const SHOW_META_LINES = { ppf: false, hex: false, choice: false };

  /** 呼び名の置き場（2026-09-19 BUBU「あー、おｋ　それでいいよ」）。
   *  冒頭の宣言文を消すと {name} の出番がそこしか無かった＝名前が画面から消えるので、
   *  「あなたはいま〜にいます」の行の頭へ移した（BUBU が実物で見た形＝「ぼんさん、あなたはいま、…」）。
   *  その行が無い型（明重・中重＝重い型でスロットを外した2本）は**本文の1段落目の頭**に付ける
   *  ＝どの型でも名前は1回だけ出る。
   *  ⚠️ 元の SUMMARY を書き換えない（同じ結果を組み直すたびに名前が二重に付くため、写しを返す）。 */
  function putName(name, paras, you) {
    if (!name) return { paras, you };
    if (you) return { paras, you: name + you };
    return { paras: paras.map((t, i) => (i === 0 ? name + t : t)), you };
  }

  /* 6枚引きの6本レールの言い回し（正本＝docs/for_chattoku_text_structure_2026-09-19.md §4-5 C）。
     summary_ja.json の meta.hex に同じキーがあればそちらが優先＝文言の正本はデータ側へ移せる。 */
  const HEX_RAIL_TPL = {
    near: "すぐ先には、{x}が待っています。",
    inner: "心の奥には、{x}があります。",
    around: "まわりには、{x}があります。",
    outcome: "行き着く先に、{x}が見えています。",
  };

  /* ===== 総括エンジンv3（トーン軌道9型・部品はsummary_ja.json・正本=docs/summary_v3_writing_method.md） ===== */
  function buildSummary(drawn, spreadKey) {
    ...
    const nick = getNick();
    const name = nick ? `${nick}さん、` : "";
    if (spreadKey === "choice") {
      /* ふたつの道（2026-09-19）：型キー＝2枚のtoneを明<中<重の順に並べた6型（patterns_choice）。
         レールは各道の sum_future（「〜が見えています」用の名詞句）をふたつ並べる */
      const order = { 明: 0, 中: 1, 重: 2 };
      const key = [sel[0].tone, sel[1].tone].sort((a, b) => order[a] - order[b]).join("");
      const pattern = (SUMMARY.patterns_choice || {})[key];
      const m = SUMMARY.meta.choice;
      if (!pattern || !m) { window.TOI_DEBUG.fail = "ふたつの道パターン不一致"; return null; }
      window.TOI_DEBUG.fail = null;
      const rail = m.rail.replace("{a}", sel[0].sum_future).replace("{b}", sel[1].sum_future);
      const echoLines = rail.split("。").filter(Boolean).map((t) => t + "。");
      const show = SHOW_META_LINES.choice;
      const nm = putName(name, pattern.paras, pattern.you);
      return { title: "ふたつの道からのメッセージ", opener: show ? m.opener.replace("{name}", name) : "", echoLines, echoTail: show ? m.echo_tail : "", paras: nm.paras, you: nm.you, close: pattern.close };
    }
    if (spreadKey === "hex") {
      /* 6枚は「内側とまわりのずれ」＝3枚の時間の軌道とは別の型（2026-09-19 BUBU「3枚と6枚が同じだったら意味がない💦」）。
         型キー＝心の奥(3)のtone × まわり(4)のtone。要約は引いた6枚が全部出る6本レール。
         ⚠️ 部品（sum_inner / sum_around）と patterns_hex はちゃっトクの原稿が揃ってから入る＝
         **無い間は下の3本レールのまま動く**（ここで null を返して落ちる）。データが揃った瞬間に6本へ切り替わる。 */
      const hx = hexSummary(sel, name);
      if (hx) return hx;
    }
    /* 3枚＝過去(0)・現在(1)・未来(2)／6枚＝過去(0)・現在(1)・行き先(5)。型キー＝最初のtone＋最後のtone（2026-09-19 6枚へ拡張） */
    const first = sel[0], mid = sel[1], last = sel[sel.length - 1];
    const pattern = SUMMARY.patterns[first.tone + last.tone];
    if (!pattern) { window.TOI_DEBUG.fail = "パターン不一致"; return null; }
    window.TOI_DEBUG.fail = null;
    const n = String(drawn.length);
    const opener = SUMMARY.meta.opener.replace("{name}", name).replace("{n}", n);
    /* 三文式＋3レール（2026-07-17確定）：現在は行き先が重い型なら影側変種、未来レールは型のfut_railで選ぶ */
    const now = last.tone === "重" ? mid.sum_now_heavy : mid.sum_now_light;
    const futTpl = pattern.fut_rail === "wait" ? SUMMARY.meta.fut_wait : SUMMARY.meta.fut_see;
    const echoLines = [first.sum_past, now, futTpl.replace("{x}", last.sum_future)];
    const show = SHOW_META_LINES[spreadKey] !== false;
    const nm = putName(name, pattern.paras, pattern.you);
    return { title: `${n}枚からのメッセージ`, opener: show ? opener : "", echoLines, echoTail: show ? SUMMARY.meta.echo_tail.replace("{n}", n) : "", paras: nm.paras, you: nm.you, close: pattern.close };
  }

  /** 6枚引きの総括（6本レール・型キー＝心の奥tone×まわりtone）。
   *  部品か型がひとつでも欠けていたら null を返す＝呼び手が今までの3本レールに落ちる。 */
  function hexSummary(sel, name) {
    const inner = sel[3], around = sel[4], goal = sel[5];
    const pattern = (SUMMARY.patterns_hex || {})[inner.tone + around.tone];
    if (!pattern) return null;
    if (!inner.sum_inner || !around.sum_around) return null;
    if (!sel[2].sum_future || !goal.sum_future) return null;
    const t = Object.assign({}, HEX_RAIL_TPL, SUMMARY.meta.hex || {});
    /* 現在のレールは「行き先が重いなら影側」＝3枚と同じ決め方（行き先＝6枚目） */
    const now = goal.tone === "重" ? sel[1].sum_now_heavy : sel[1].sum_now_light;
    const echoLines = [
      sel[0].sum_past,
      now,
      t.near.replace("{x}", sel[2].sum_future),
      t.inner.replace("{x}", inner.sum_inner),
      t.around.replace("{x}", around.sum_around),
      t.outcome.replace("{x}", goal.sum_future),
    ];
    window.TOI_DEBUG.fail = null;
    window.TOI_DEBUG.hexKey = inner.tone + around.tone;
    const show = SHOW_META_LINES.hex;
    const nm = putName(name, pattern.paras, pattern.you);
    return {
      title: "6枚からのメッセージ",
      opener: show ? SUMMARY.meta.opener.replace("{name}", name).replace("{n}", "6") : "",
      echoLines,
      echoTail: show ? SUMMARY.meta.echo_tail.replace("{n}", "6") : "",
      paras: nm.paras, you: nm.you, close: pattern.close,
    };
  }
  function summaryHtml(drawn, spreadKey) {
    const s = buildSummary(drawn, spreadKey);
    if (!s) return "";
    ...
    return `<section class="summary-box" id="summaryBox">
      ...
      ${s.opener ? `<p class="summary-text">${sentLines(s.opener)}</p>` : ""}
      <p class="summary-text summary-rails">${s.echoLines.map((t) => `<span class="sent">${t}</span>`).join("")}</p>
      ${s.echoTail ? `<p class="summary-text">${s.echoTail}</p>` : ""}
  ...
  function renderResult() {
    ...
    if (spreadKey === "today") {
      ...
      /* 1枚の本文はクロ素材のフル解説へ配線替え（2026-09-05 BUBU「やっちゃおう」）。
         旧 c.present（7/15初版・耳ルール以前の文）は VOICE が引けない時だけの保険 */
      const vy = VOICE && VOICE[c.id];
      html += vy ? blockParas("YES / NO", "いまの空気", c, vy.full, 0)
                 : block("YES / NO", "いまの空気", c, c.present, 0);
    } else if (spreadKey === "choice") {
      drawn.forEach((c, i) => { html += blockPoem(spec.pos[i][0], spec.pos[i][1], c, i); });
      html += summaryHtml(drawn, "choice");
      ...
      html += summaryHtml(drawn, "ppf");
    } else if (spreadKey === "hex") {
      drawn.forEach((c, i) => { html += blockPoem(spec.pos[i][0], spec.pos[i][1], c, i, spec.pos[i][2]); });
      /* 6枚にも総括を出す（2026-09-19・設計書§7-5「他スプレッドへの反映」の未着手分） */
      html += summaryHtml(drawn, "hex");
    }
    readingBody.innerHTML = html;
    renderDeepDive(spreadKey);
    // 今日の一枚は日替わり固定なので「もういちど引く」を出さない（同じカードが出て混乱するため）
    $("#againBtn").classList.toggle("hidden", spreadKey === "today");
  }

  /* 余韻ゾーンの送客枠（読み解きの下・「もういちど引く」の上）
     出す条件は3つ全部：enabled が true／そのスプレッドに枠が割り当ててある／URL が入っている。
     1つでも欠けたら hidden のまま＝何も出ない（＝URLを入れるまでサイトの見た目は変わらない）。
     ［PR］表記は必須（2023年ステマ規制・景表法）。ここを外さないこと。 */
  function renderDeepDive(spreadKey) {
    const box = document.getElementById("deepDive");
    if (!box) return;
    box.hidden = true;
    box.innerHTML = "";
    if (!AFF || AFF.enabled !== true) return;
    const key = (AFF.spread_map || {})[spreadKey];
    if (!key) return; // 今日の一枚・YES/NO＝毎日来る場所なので出さない
    const slot = (AFF.slots || {})[key];
    if (!slot || !slot.url) return;
    const esc = (t) => String(t == null ? "" : t).replace(/[&<>"]/g, (m) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[m]));
    // href は http/https だけ通す。JSON に貼り間違えた文字列がそのままリンクにならないように
    const safeUrl = (u) => (/^https?:\/\//i.test(String(u == null ? "" : u).trim()) ? String(u).trim() : "");
    let h = "";
    if (slot.label) h += `<p class="deep-label">${esc(slot.label)}<span class="deep-pr">［PR］</span></p>`;
    if (slot.text) h += `<p class="deep-text">${esc(slot.text)}</p>`;
    const href = safeUrl(slot.url);
    if (!href) return; // http/https 以外は枠ごと出さない
    h += `<a class="deep-link" href="${esc(href)}" target="_blank" rel="sponsored nofollow noopener">${esc(slot.cta || "くわしく見る")}</a>`;
    // ASP の表示回数を数える1×1画像（afb の原稿タグに付いてくる物）。URL と同じく http/https だけ
    const imgUrl = safeUrl(slot.img);
    if (imgUrl) h += `<img src="${esc(imgUrl)}" width="1" height="1" alt="" style="border:none;position:absolute;left:-9999px" />`;
    box.innerHTML = h;
    box.hidden = false;
  }
  ...
  function landFromShare(cid) {
    ...
    /* シェア着地の本文もクロ素材のフル解説へ配線替え（2026-09-05 BUBU「やっちゃおう」）。
       決め文（kime）を旧 one の席に置き、旧 c.present は VOICE が引けない時だけの保険 */
    const vs = VOICE && VOICE[c.id];
    readingBody.innerHTML =
      `<p class="one-line" style="animation:rise-in 0.9s ease both">「${vs ? vs.kime : c.one}」</p>` +
      (vs ? blockParas("CARD", "この一枚", c, vs.full, 0) : block("CARD", "この一枚", c, c.present, 0)) +
  ...
  Promise.all([
    ...
    fetch("/data/summary_ja.json?v=1789786103", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
    fetch("/data/kuro_voice.json?v=1789786103", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
    fetch("/data/affiliates.json?v=1789786103", { cache: "no-store" }).then((r) => (r.ok ? r.json() : null)).catch(() => null),
  ])
    .then(([data, summary, voice, aff]) => {
      CARDS = data;
      SUMMARY = summary;
      VOICE = voice;
      AFF = aff;
